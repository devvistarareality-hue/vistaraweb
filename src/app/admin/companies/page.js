...
export default function CompanyManagementPage() {
  ...
  const handleDeactivate = (c) => {
    setDialog({ open: true, title: 'Deactivate Company', message: `Deactivate "${c.name}"? Users of this company will no longer be able to log in.`, confirmLabel: 'Deactivate', confirmColor: '#D98A1F',
  ...
  const handleActivate = (c) => {
    setDialog({ open: true, title: 'Reactivate Company', message: `Reactivate "${c.name}"? Users will regain access.`, confirmLabel: 'Activate', confirmColor: '#23874A',
      onConfirm: () => { dispatch(updateCompany(c.id, { is_active: true })); showToast(`"${c.name}" reactivated.`, 'success'); closeDialog(); } });
  };
  const handleDelete = (c) => {
    setDialog({ open: true, title: 'Delete Company', message: `Permanently delete "${c.name}"? This cannot be undone and all related data will be removed.`, confirmLabel: 'Delete', confirmColor: '#D9434B',
    ...
    <div style={s.page}>
        ...
        <div style={s.tableWrap}>
          <table style={s.table}>
            ...
            <tbody>
              ...
              {companies.length === 0 && !loading && (
                <tr><td colSpan={6} style={{ ...s.td, textAlign: 'center', color: '#6E7278', padding: '36px 16px' }}>No companies found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showCreate && (
        <div style={s.overlay} onClick={() => setShowCreate(false)}>
          <div style={{ backgroundColor: '#fff', borderRadius: 20, width: '90%', maxWidth: 520, boxShadow: '0 24px 80px rgba(29,29,31,0.18)', overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>

            {/* Gradient Header */}
            <div style={{ background: '#1D1D1F', padding: '22px 24px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
            ...
            <form onSubmit={handleCreate} style={{ padding: '22px 24px 24px' }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#9A9EA5', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>Company Details</div>
              <div style={{ marginBottom: 14 }}>
                <label style={mLbl}>Company Code <span style={{ color: '#D9434B' }}>*</span></label>
                ...
                <p style={{ fontSize: 11, color: '#9A9EA5', marginTop: 5 }}>Unique code employees use to log in. Cannot be changed easily.</p>
              </div>
              <div style={{ marginBottom: 14 }}>
                <label style={mLbl}>Company Name <span style={{ color: '#D9434B' }}>*</span></label>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                <button type="button" onClick={() => setShowCreate(false)} style={{ padding: '10px 20px', backgroundColor: '#F4F5F7', color: '#55585E', border: 'none', borderRadius: 14, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                <button type="submit" disabled={creating} style={{ padding: '10px 24px', background: '#1D1D1F', color: '#fff', border: 'none', borderRadius: 14, fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: creating ? 0.7 : 1, minWidth: 140 }}>
...
const s = {
  ...
  pageSub:    { fontSize: 13, color: '#6E7278' },
  ...
  tableWrap:  { overflowX: 'auto', backgroundColor: '#fff', borderRadius: 20, boxShadow: '0 4px 12px rgba(140,148,160,0.18)' },
  table:      { width: '100%', borderCollapse: 'collapse' },
  th:         { padding: '14px 16px', fontSize: 11, fontWeight: 700, color: '#6E7278', textAlign: 'left', letterSpacing: 0.5, borderBottom: '1px solid #ECEEF0', whiteSpace: 'nowrap' },
  tr:         { borderBottom: '1px solid #ECEEF0' },
  ...
  nameText:   { fontWeight: 600, color: '#1D1D1F' },
  muted:      { color: '#6E7278' },
  statusPill: { padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 700, color: '#fff' },
  rowActions: { display: 'flex', gap: 6 },
  editBtn:    { padding: '5px 12px', backgroundColor: '#E6F2FF', color: '#2F6DB5', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' },
  inlineInput:{ padding: '6px 10px', border: '1.5px solid #DFE2E6', borderRadius: 6, fontSize: 13, width: '100%' },
  saveBtn:    { padding: '5px 14px', backgroundColor: '#1D1D1F', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' },
  cancelBtn:  { padding: '5px 12px', backgroundColor: '#F4F5F7', color: '#6E7278', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' },
