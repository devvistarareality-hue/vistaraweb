...
export default function UserManagementPage() {
  ...
  const handleDeactivate = (u) => {
    if (confirm(`Deactivate ${u.name}? They will no longer be able to log in.`)) {
      dispatch(deleteUser(u.id));
      showToast(`${u.name} deactivated.`, 'info');
    }
  };
    ...
    <div style={s.page}>
      ...
      <div style={s.pageHeader}>
        <div>
          ...
          <p style={s.pageSub}>{users.length} user{users.length !== 1 ? 's' : ''} in your company</p>
        </div>
            ...
            <tbody>
              {filtered.map((u) => (
                <tr key={u.id} style={s.tr}>
                  <td style={s.td}><code style={s.codePill}>{u.user_code}</code></td>
                  <td style={s.td}><span style={s.nameText}>{u.name}</span></td>
                  ...
                  <td style={s.td}>
                    <span style={{ ...s.statusPill, backgroundColor: u.is_active ? COLORS.success : COLORS.error }}>
              ...
              {filtered.length === 0 && !loading && (
                <tr>
                  <td colSpan={7} style={{ ...s.td, textAlign: 'center', color: '#8492A6', padding: '36px 16px' }}>
                    No users found.
                  </td>
                </tr>
              )}
...
const s = {
  ...
  muted:      { color: '#8492A6' },
  rolePill:   { backgroundColor: '#E8EEFF', color: '#3D5AFE', padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 700 },
  ...
  deactBtn:   { padding: '5px 12px', backgroundColor: '#FFEEEE', color: '#EF4444', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' },
  ...
  saveBtn:   { padding: '10px 24px', backgroundColor: '#182350', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer' },
};
