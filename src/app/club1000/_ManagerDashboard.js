    ...
    <div style={{ padding: '28px 32px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#8492A6', marginTop: 4 }}>Investment portfolio and returns tracking — manager view</p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
          <div style={{ display: 'flex', gap: 10 }}>
            <Link href="/club1000/schemes" style={{ textDecoration: 'none' }}>
              <button style={{ padding: '10px 18px', background: '#F3F4F6', color: '#334155', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Manage Schemes</button>
            </Link>
          ...
          {!loading && !schemes.length && (
            <div style={{ fontSize: 11, color: '#E65100' }}>Create a scheme first — the button unlocks once one exists.</div>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div style={{ marginTop: 4, display: 'flex', flexWrap: 'wrap', gap: 14 }}>
            ...
            <StatCard label="Pending Payouts" value={`${stats?.pending_payout_count ?? 0} · ${fmtMoney(stats?.pending_payout_amount)}`} accent="#E65100" href="/club1000/payouts?status=pending" />
            <StatCard label="Paid Payouts" value={`${stats?.paid_payout_count ?? 0} · ${fmtMoney(stats?.paid_payout_amount)}`} accent="#2E7D32" href="/club1000/payouts?status=paid" />
          </div>

          <div style={{ marginTop: 28 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#1A1A2E', marginBottom: 12 }}>By Scheme</div>
            <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #EDF1F7', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                ...
                <tbody>
                  {(stats?.by_scheme || []).length === 0 ? (
                    <tr><td colSpan={3} style={{ ...td, textAlign: 'center', color: '#8492A6' }}>No investors yet.</td></tr>
