...
const APPROVAL_COLORS = {
  ...
  approved: { bg: '#E9FBEA', fg: '#23874A' },
  rejected: { bg: '#FDECEC', fg: '#D9434B' },
};
const TABS = [['pending', 'Pending'], ['approved', 'Approved'], ['rejected', 'Rejected'], ['all', 'All']];

function ApprovalBadge({ approvalStatus }) {
  const c = APPROVAL_COLORS[approvalStatus] || { bg: '#F4F5F7', fg: '#55585E' };
...
export default function InvestorApprovalsPage() {
  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      <h1 style={{ fontSize: 24, fontWeight: 800, color: '#1D1D1F' }}>Investor Approvals</h1>
      <p style={{ fontSize: 13, color: '#6E7278', marginTop: 4 }}>{investors.length} {tab === 'all' ? 'total' : tab} investor{investors.length === 1 ? '' : 's'}</p>
        ...
        <div style={{ background: '#fff', borderRadius: 18, padding: '14px 18px', marginTop: 18, border: '1px solid #ECEEF0' }}>
          ...
          {cfgOpen && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 12, color: '#6E7278', marginBottom: 8 }}>For each scheme, pick the managers who approve investors added under it. They get a notification on each new submission for that scheme.</div>
              {managers.length === 0 ? <div style={{ fontSize: 13, color: '#6E7278' }}>No managers in this company.</div> : schemes.map((s) => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 0', borderTop: '1px solid #F4F5F7' }}>
                  <div style={{ width: 180, minWidth: 180, fontSize: 13, fontWeight: 700, color: '#1D1D1F' }}>{s.name}</div>
      ...
      <div style={{ marginTop: 14, display: 'flex', gap: 6 }}>
        {TABS.map(([k, label]) => (
          <button key={k} onClick={() => setTab(k)} style={{ padding: '7px 14px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer',
            background: tab === k ? TEAL : '#ECEEF0', color: tab === k ? '#fff' : '#6E7278' }}>{label}</button>
        ))}
      </div>

      {loading ? <p style={{ color: '#6E7278', marginTop: 18 }}>Loading…</p> : investors.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 18, padding: 40, textAlign: 'center', color: '#6E7278', marginTop: 18, border: '1px solid #ECEEF0' }}>{search ? 'No investors match your search.' : 'No investors here.'}</div>
      ) : (
              ...
              {investors.map((inv) => {
                ...
                <tr key={inv.id}>
                  <td style={{ ...td, fontWeight: 600 }}>
                    {inv.name}
                    {inv.revision_no > 0 && <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 800, color: accent, background: isRenewal ? '#FFF3E0' : '#E6F2FF', padding: '2px 7px', borderRadius: 20 }}>{isRenewal ? 'RENEW' : 'R'}{isRenewal ? '' : inv.revision_no}</span>}
                    {isRevision && <div style={{ fontSize: 11, fontWeight: 500, color: '#6E7278', marginTop: 3 }}>Proposed: {revisionSummary(inv)}</div>}
                  </td>
