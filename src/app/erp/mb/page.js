...
const STATUS_COLORS = {
  ...
  'Submitted': { bg: '#EEF0FF', text: '#3D5AFE' },
...
export default function MBListPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const res   = await fetch(ERP_EXECUTION.mbs, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  const statuses = ['All', 'Draft', 'Submitted', 'Certified', 'Billed'];
  const filtered = mbs.filter((m) =>
  ...
  return (
    <div style={s.page}>
      <div style={s.pageHeader}>
        <div>
          <div style={s.breadcrumb}>ERP › Measurement Books</div>
          <h1 style={s.pageTitle}>Measurement Books</h1>
        </div>
      ...
      <div style={s.toolbar}>
        <input style={s.search} placeholder="Search MB or project..." value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      <div style={s.tableWrap}>
        {loading ? (
          ...
          <div style={s.empty}>No measurement books found.</div>
        ) : (
          <table style={s.table}>
            <thead>
              <tr>{['MB No.', 'Project', 'Prepared By', 'MB Date', 'Certified By', 'Status'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.map((mb) => {
                const c = STATUS_COLORS[mb.status] || { bg: '#F5F6FA', text: '#8492A6' };
                return (
                  <tr key={mb.id} style={s.tr}>
                    <td style={{ ...s.td, fontWeight: 700, color: '#1A1A2E' }}>{mb.mb_no}</td>
                    <td style={s.td}>{mb.project_name}</td>
                    <td style={s.td}>{mb.prepared_by_name}</td>
                    <td style={s.td}>{mb.mb_date}</td>
                    <td style={s.td}>{mb.certified_by_name || '—'}</td>
                    <td style={s.td}><span style={{ ...s.badge, backgroundColor: c.bg, color: c.text }}>{mb.status}</span></td>
                  </tr>
                );
              })}
            </tbody>
