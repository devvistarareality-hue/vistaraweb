...
export default function POListPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const res   = await fetch(ERP_PURCHASE.pos, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  const statuses = ['All', 'Draft', 'Confirmed', 'Dispatched', 'Closed', 'Cancelled'];
  ...
  return (
    <div style={s.page}>
      <div style={s.pageHeader}>
        <div>
          <div style={s.breadcrumb}>ERP › Purchase Orders</div>
          <h1 style={s.pageTitle}>Purchase Orders</h1>
        </div>
        <div style={s.headerRight}>
          <span style={s.countChip}>{filtered.length} records</span>
          <button onClick={load} style={s.refreshBtn}>↻ Refresh</button>
          <button onClick={() => router.push('/erp/po/create')} style={s.newBtn}>+ New PO</button>
        </div>
      </div>

      <div style={s.toolbar}>
        <input style={s.search} placeholder="Search PO, vendor or project..." value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      <div style={s.tableWrap}>
        {loading ? (
          ...
          <div style={s.empty}>No purchase orders found.</div>
        ) : (
          <table style={s.table}>
            <thead>
              <tr>{['PO No.', 'Project', 'Vendor', 'PO Date', 'Delivery', 'Lines', 'Status'].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.map((po) => {
                const c = STATUS_COLORS[po.status] || { bg: '#F5F6FA', text: '#8492A6' };
                return (
                  <tr key={po.id} style={s.tr}>
                    <td style={{ ...s.td, fontWeight: 700, color: '#1A1A2E' }}>{po.po_no}</td>
                    <td style={s.td}>{po.project_name}</td>
                    <td style={s.td}>{po.vendor_name}</td>
                    <td style={s.td}>{po.po_date}</td>
                    <td style={s.td}>{po.delivery_date || '—'}</td>
                    <td style={{ ...s.td, textAlign: 'center' }}>{po.line_count}</td>
                    <td style={s.td}><span style={{ ...s.badge, backgroundColor: c.bg, color: c.text }}>{po.status}</span></td>
                  </tr>
                );
              })}
            </tbody>
...
const s = {
  ...
  newBtn:     { padding: '8px 18px', borderRadius: 10, border: 'none', backgroundColor: '#E65100', fontSize: 13, fontWeight: 700, color: '#fff', cursor: 'pointer' },
