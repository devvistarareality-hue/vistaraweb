...
export default function KioskBookPage() {
  return (
    <div style={{ minHeight: '100vh', position: 'relative',
      background: 'radial-gradient(1200px 600px at 15% -10%,var(--accent-softer) 0%,transparent 60%),radial-gradient(1000px 500px at 110% 10%,var(--accent-soft) 0%,transparent 55%),linear-gradient(160deg,var(--accent-softer) 0%,var(--surface-2) 100%)' }}>
      {/* Brand header (matches the kiosk landing) */}
      <header style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 40px',
        background: 'var(--glass)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(var(--ink-rgb),0.06)' }}>
        <div style={{ width: 44, height: 44, borderRadius: 13, display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontWeight: 800, fontSize: 20, color: '#fff', background: 'var(--strong)', boxShadow: '0 6px 16px rgba(47,109,181,0.28)' }}>V</div>
        <div>
          <div style={{ fontSize: 19, fontWeight: 800, color: 'var(--text)', letterSpacing: -0.3 }}>Vistara Realty</div>
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>Self-Service Booking Kiosk</div>
        </div>
      </header>
