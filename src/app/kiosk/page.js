...
const KSTATUS = {
  available: { dot: 'var(--success)', label: 'Available' },
  sold:      { dot: 'var(--danger)', label: 'Sold' },
  hold:      { dot: 'var(--warning-2)', label: 'On Hold' },
};
              ...
              <div className="k-floors">
                ...
                <div className="k-stats">
                  {['available', 'hold', 'sold'].map((k) => {
                    ...
                    return (
                      <div key={k} className="k-stat" style={{ borderColor: `color-mix(in srgb, ${KSTATUS[k].dot} 33%, transparent)` }}>
...
const Style = () => (
  <style>{`
  .k-root{min-height:100vh;position:relative;overflow-x:hidden;font-family:inherit;color:var(--text);
    background:radial-gradient(1200px 600px at 15% -10%,var(--accent-soft) 0%,transparent 60%),
               radial-gradient(1000px 500px at 110% 10%,var(--accent-soft) 0%,transparent 55%),
               linear-gradient(160deg,var(--accent-softer) 0%,var(--surface-2) 100%);}
  .k-bg{position:fixed;inset:0;pointer-events:none;
    background:radial-gradient(600px 300px at 90% 100%,rgba(47,109,181,.06),transparent 70%);}
  ...
  .k-logo{width:46px;height:46px;border-radius:14px;display:flex;align-items:center;justify-content:center;
    font-weight:800;font-size:20px;color:#fff;background:var(--strong);
    box-shadow:0 6px 16px rgba(47,109,181,.28)}
  .k-brand-name{font-size:19px;font-weight:800;letter-spacing:-.3px}
  .k-brand-sub{font-size:12px;color:var(--muted);margin-top:1px}
  ...
  .k-signout{display:inline-flex;align-items:center;gap:6px;padding:9px 16px;border-radius:10px;cursor:pointer;
    font-size:13px;font-weight:700;color:var(--danger);background:var(--danger-soft);border:1.5px solid var(--danger-2);transition:.15s}
  .k-signout:hover{background:var(--danger-soft)}
  ...
  .k-step-dot{width:26px;height:26px;border-radius:50%;display:flex;align-items:center;justify-content:center;
    font-size:12px;font-weight:800;background:var(--surface-3);color:var(--faint);transition:.25s}
  .k-step-dot.active{background:var(--primary);color:#fff;box-shadow:0 4px 12px rgba(47,109,181,.4)}
  .k-step-dot.done{background:var(--success-solid);color:#fff}
  .k-step-label{font-size:13px;font-weight:600;color:var(--faint)}
  .k-step-label.active{color:var(--text)}
  .k-step-bar{width:34px;height:2px;background:var(--border);margin:0 12px;border-radius:2px}
  ...
  .k-hero-sub{font-size:17px;color:var(--text-3);margin:0 0 30px}
  ...
  .k-card{padding:0;border:none;background:var(--surface);border-radius:22px;overflow:hidden;cursor:pointer;text-align:left;
    box-shadow:0 8px 30px rgba(var(--ink-rgb),.08);transition:transform .22s cubic-bezier(.2,.8,.2,1),box-shadow .22s}
  .k-card:hover{transform:translateY(-6px);box-shadow:0 20px 44px rgba(var(--ink-rgb),.16)}
  ...
  .k-card-ph{width:100%;height:100%;background:linear-gradient(135deg,var(--blue-2),var(--accent-soft))}
  .k-card-veil{position:absolute;inset:0;background:linear-gradient(to top,rgba(var(--ink-rgb),.72),transparent 55%)}
  ...
  .k-card-loc{font-size:13px;color:var(--muted)}
  .k-card-meta{display:flex;flex-wrap:wrap;gap:8px;margin:12px 0}
  .k-tag{font-size:12px;font-weight:700;color:var(--accent);background:var(--accent-soft);border-radius:20px;padding:5px 12px}
  .k-tag.ghost{color:var(--text-3);background:var(--surface-2)}
  .k-card-cta{font-size:14px;font-weight:800;color:var(--accent)}

  .k-panel{background:var(--surface);border-radius:24px;padding:30px 32px;box-shadow:0 10px 34px rgba(var(--ink-rgb),.08)}
  .k-narrow{max-width:560px}
  .k-back{background:none;border:none;color:var(--text-3);font-size:14px;font-weight:700;cursor:pointer;padding:0;margin-bottom:12px}
  .k-master{width:100%;max-height:380px;object-fit:contain;background:var(--surface-2);border-radius:16px;margin-bottom:22px}
  .k-maphead{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:12px}
  .k-legend{display:flex;gap:16px;font-size:12px;font-weight:600;color:var(--text-3)}
  ...
  .k-floors-l{font-size:12px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted);margin-right:2px}
  .k-floor{padding:8px 14px;border-radius:20px;border:1.5px solid var(--border);background:var(--surface);color:var(--text-2);font-size:13px;font-weight:700;cursor:pointer;transition:.15s}
  .k-floor:hover{border-color:var(--blue-2)}
  .k-floor.on{border-color:var(--accent);background:var(--accent-soft);color:var(--accent)}
  .k-stats{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px}
  .k-stat{display:flex;align-items:center;gap:8px;background:var(--surface);border:1.5px solid var(--surface-3);border-radius:14px;padding:10px 16px;flex:1;min-width:150px}
  ...
  .k-tip-type{background:rgba(47,109,181,0.25);color:#A2D2FF;border:1px solid rgba(47,109,181,0.5)}
  .k-tip-sz{color:var(--warning-2);font-size:11px;font-weight:600;margin-top:5px}
  .k-tip-fac{color:#A2D2FF;font-size:11px;font-weight:600;margin-top:3px}
  .k-tip-ter{color:#A4F5A6;font-size:11px;font-weight:600;margin-top:3px}
  .k-tip-agent{color:var(--border);font-size:11px;font-weight:600;margin-top:3px}
  ...
  .k-note{font-size:15px;color:var(--text-2);line-height:1.5;margin:0 0 18px}
  .k-note.center{text-align:center}
  .k-label{display:block;font-size:12px;font-weight:800;letter-spacing:.4px;text-transform:uppercase;color:var(--muted);margin:18px 0 8px}
  ...
  .k-chip{display:flex;flex-direction:column;align-items:flex-start;gap:2px;min-width:96px;padding:14px 18px;border-radius:16px;
    border:1.5px solid var(--border);background:var(--surface);color:var(--text-2);cursor:pointer;transition:.15s}
  .k-chip:hover{border-color:var(--blue-2)}
  .k-chip.on{border-color:var(--accent);background:var(--accent-soft);color:var(--accent);box-shadow:0 6px 16px rgba(47,109,181,.18)}
  ...
  .k-plot{display:flex;flex-direction:column;align-items:center;gap:2px;min-width:92px;padding:14px 12px;border-radius:16px;
    border:1.5px solid var(--border);background:var(--surface);color:var(--text-2);cursor:pointer;transition:.15s}
  .k-plot:hover{border-color:var(--blue-2);transform:translateY(-2px)}
  .k-plot.on{border-color:var(--accent);background:var(--accent-soft);color:var(--accent);box-shadow:0 8px 18px rgba(47,109,181,.2)}
  .k-plot-no{font-size:17px;font-weight:800}
  .k-plot-sz{font-size:11px;opacity:.75}

  .k-stepper{display:inline-flex;align-items:center;border:1.5px solid var(--border);border-radius:14px;overflow:hidden;background:var(--surface)}
  .k-stepper button{width:52px;height:52px;border:none;background:var(--surface-2);font-size:24px;font-weight:700;color:var(--accent);cursor:pointer}
  .k-stepper button:active{background:var(--accent-soft)}
  .k-stepper input{width:80px;height:52px;border:none;text-align:center;font-size:18px;font-weight:700;color:var(--text);outline:none}
  .k-summary{margin-top:16px;font-size:15px;color:var(--text-2)}
  .k-summary b{color:var(--accent)}

  .k-input{width:100%;height:54px;padding:0 16px;border-radius:14px;border:1.5px solid var(--border);font-size:16px;
    box-sizing:border-box;outline:none;background:var(--surface);transition:.15s}
  .k-input:focus{border-color:var(--accent);box-shadow:0 0 0 4px rgba(47,109,181,.12)}
  ...
  .k-primary{height:56px;padding:0 34px;border:none;border-radius:16px;font-size:17px;font-weight:800;color:#fff;cursor:pointer;
    background:var(--strong);box-shadow:0 10px 24px rgba(47,109,181,.32);transition:.15s}
  ...
  .k-fine{font-size:12px;color:var(--faint);margin-top:14px;text-align:center}
  .k-err{margin-top:16px;background:var(--danger-soft);border:1px solid var(--danger-2);border-radius:12px;padding:12px 16px;font-size:14px;color:var(--danger)}

  .k-done{max-width:560px;margin:20px auto 0;text-align:center;background:var(--surface);border-radius:24px;padding:44px 32px;
    box-shadow:0 10px 34px rgba(var(--ink-rgb),.08)}
  .k-check{width:92px;height:92px;border-radius:50%;background:var(--success-soft);color:var(--success);font-size:48px;
    display:flex;align-items:center;justify-content:center;margin:0 auto 22px;
    box-shadow:0 10px 26px rgba(35,135,74,.22)}
  .k-done-msg{font-size:17px;color:var(--text-2);margin:10px 0 4px}

  .k-empty{background:var(--surface);border-radius:20px;padding:44px;text-align:center;color:var(--text-3);font-size:16px;line-height:1.6;
    box-shadow:0 8px 26px rgba(var(--ink-rgb),.06)}
  .k-spin-wrap{display:flex;justify-content:center;padding:80px}
  .k-spin{width:38px;height:38px;border:3px solid var(--blue-2);border-top-color:var(--accent);border-radius:50%;animation:kspin .8s linear infinite}
  ...
  .k-exit{position:fixed;bottom:14px;right:16px;background:transparent;color:var(--faint);border:none;font-size:11px;cursor:pointer;z-index:5}
  .k-exit:hover{color:var(--text-3)}
