  ...
  async function recordClosure() {
    ...
    try {
      const res = await fetch(SALES_ENDPOINTS.closures, {
        ...
        body: JSON.stringify({
          lead: sv.lead, site_visit: sv.id, project: project.id,
      ...
      if (res.ok) {
        ...
        // NOTE: the booked unit is intentionally NOT auto-marked Sold yet — pending
        // product decision. To enable, PATCH SALES_ENDPOINTS.plot(plot.id) { status: 'sold' }.
    ...
    <div onClick={onClose} style={overlay}>
      <div onClick={(e) => e.stopPropagation()} style={panel}>
        ...
        <div style={{ padding: 20 }}>
          ...
          {sv ? (
            !showForm ? (
              <button onClick={() => { setErr(''); setShowForm(true); }} style={primaryBtn}>Record Closure for Unit {plot.number}</button>
            ) : (
              <div style={{ borderTop: '1px solid #F0F3FA', paddingTop: 16 }}>
                <div style={{ fontSize: 13, color: '#8492A6', marginBottom: 12 }}>{sv.lead_name} · {sv.lead_phone}</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 14px', marginBottom: 12 }}>
                  ...
                  <Field label="Unit Type">
                    <input value={closure.unit_type} onChange={(e) => setClosure({ ...closure, unit_type: e.target.value })} style={inp} />
                  </Field>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 14 }}>
                  ...
                  <button onClick={recordClosure} disabled={saving} style={{ ...primaryBtn, width: 'auto', opacity: saving ? 0.7 : 1 }}>{saving ? 'Saving…' : 'Save Closure'}</button>
                </div>
              </div>
            )
          ) : (
