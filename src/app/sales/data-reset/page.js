    ...
    <div style={{ padding: '24px 28px', maxWidth: 680 }}>
      ...
      <div className="nx-card" style={{ background: 'var(--surface)', border: '1px solid var(--surface-3)', borderRadius: 18, padding: 18, marginBottom: 18 }}>
              ...
              return (
                <label key={k} title={isImplied ? 'Deleted together with Leads' : ''}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, fontSize: 13,
                    color: 'var(--text-2)', cursor: isImplied ? 'not-allowed' : 'pointer', padding: '5px 0', opacity: isImplied ? 0.7 : 1 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    <input type="checkbox" checked={checked} disabled={isImplied} onChange={() => toggle(k)} />
                    {label}{isImplied && <span style={{ fontSize: 10, color: 'var(--faint)' }}>(via Leads)</span>}
                  </span>
                  <span style={{ fontWeight: 800, color: (counts?.[k] || 0) > 0 ? (k === 'plots_to_reset' ? 'var(--success)' : RED) : 'var(--faint)' }}>{counts?.[k] ?? 0}</span>
                </label>
              );
            })}
          </div>
        ...
        {!loading && (selected.has('leads') || selected.has('bookings')) && (
          <p style={{ fontSize: 11, color: 'var(--faint)', marginTop: 8, lineHeight: 1.6 }}>
            {selected.has('leads') && (
      ...
      <div style={{ background: 'var(--danger-soft)', border: `1.5px solid ${RED}`, borderRadius: 18, padding: 18 }}>
        ...
        {!!msg && <p style={{ marginTop: 12, fontSize: 13, fontWeight: 600, color: msg[0] === '✅' ? 'var(--success)' : RED, display: 'flex', alignItems: 'center', gap: 6 }}><Icon name={msg[0] === '✅' ? 'check-circle' : 'alert'} />{msg.replace(/^[^\p{L}\p{N}]+/u, '')}</p>}
      </div>
    </div>
  );
}
