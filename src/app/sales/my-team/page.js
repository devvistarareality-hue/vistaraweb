...
// driven by reporting_manager. Table or graphical org-chart view.
export default function MyTeamPage() {
  ...
  useEffect(() => {
    fetch(SALES_ENDPOINTS.myTeam, { headers: authHeaders() })
      .then((r) => r.json())
  ...
  // Build the org tree. Managers root at themselves; admins/viewers (who aren't
  // in the reporting tree) root at the org top — the same chart the CMO sees.
  const tree = (() => {
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 18 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1A1A2E', marginBottom: 4 }}>My Team</h1>
          <p style={{ fontSize: 13, color: '#8492A6' }}>
            {orgView
              ? `${team.length} ${team.length === 1 ? 'person' : 'people'} across the organisation`
              : `${team.length} ${team.length === 1 ? 'person' : 'people'} reporting under you${team.length ? ` · ${directs} direct` : ''}`}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ background: '#fff', borderRadius: 14, padding: '48px 24px', textAlign: 'center', boxShadow: '0 2px 8px rgba(184,196,214,0.18)' }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: '#1A1A2E', marginBottom: 4 }}>No one reports to you yet.</p>
          <p style={{ fontSize: 13, color: '#8492A6' }}>Set their <strong>Reporting Manager</strong> to you in Team Users, and they’ll appear here.</p>
        </div>
      ) : view === 'chart' ? (
                    ...
                    <tr key={m.id} style={{ borderBottom: '1px solid #F0F3FA' }}>
                      <td style={td}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          {m.is_direct_report && <span style={directBadge}>DIRECT</span>}
                        </div>
                      </td>
      ...
      <style jsx global>{`
        ...
        .org-tree li::before, .org-tree li::after {
          ...
          width: 50%; height: 28px; border-top: 1.5px solid #C5CEDC;
        }
        ...
        .org-tree ul ul::before {
          ...
          width: 0; height: 28px; border-left: 1.5px solid #C5CEDC;
        }
        /* node card */
        .org-card {
          position: relative; display: inline-block; width: 196px; box-sizing: border-box;
          padding: 14px 16px 12px; border-radius: 12px; background: #fff;
          border: 1px solid #E6EBF4; box-shadow: 0 6px 20px rgba(90,110,150,0.12);
...
// same-designation people in a contiguous run (telecallers next to telecallers).
...
function accentFor(node) {
  ...
  if (d.includes('telecaller') || d.includes('pre-sale')) return '#0891B2'; // cyan
  if (d.includes('cp ') || d.includes('channel'))         return '#D97706'; // amber
  if (d.includes('stm') || d.includes('sales'))           return '#2563EB'; // blue
  if (d.includes('market'))                               return '#DB2777'; // pink
  if (node.role === 'Manager')                            return '#7C3AED'; // purple
  return '#64748B';                                                          // slate
}
  ...
  return (
    <li>
      ...
      {node.children?.length > 0 && (
        <ul>{node.children.map((c2) => <OrgNode key={c2.id} node={c2} />)}</ul>
      )}
    </li>
  );
}
