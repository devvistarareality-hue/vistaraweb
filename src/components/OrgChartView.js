  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 18 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1D1D1F', marginBottom: 4 }}>{title}</h1>
      ...
      {loading ? (
        <p style={{ color: '#6E7278', padding: '40px 0', textAlign: 'center' }}>Loading…</p>
      ) : team.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 18, padding: '48px 24px', textAlign: 'center', boxShadow: '0 2px 8px rgba(140,148,160,0.18)' }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: '#1D1D1F', marginBottom: 4 }}>No org chart yet.</p>
          <p style={{ fontSize: 13, color: '#6E7278' }}>Assign people to this {label ? 'department' : 'team'} and set their <strong>Reporting Manager</strong>, and they’ll appear here.</p>
        </div>
      ) : view === 'chart' ? (
        <div style={{ background: 'radial-gradient(circle at 1px 1px, #ECEEF0 1px, transparent 0) 0 0 / 22px 22px, #FAFAFB', borderRadius: 20, border: '1px solid #ECEEF0', boxShadow: 'inset 0 0 40px rgba(201,205,210,0.18)', padding: '36px 20px', overflowX: 'auto' }}>
        ...
        <>
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name, code or designation…"
            style={{ width: '100%', maxWidth: 420, height: 40, padding: '0 14px', borderRadius: 14, border: '1.5px solid #DFE2E6', fontSize: 14, outline: 'none', marginBottom: 18, boxSizing: 'border-box' }} />
          <div style={{ background: '#fff', borderRadius: 18, boxShadow: '0 2px 8px rgba(140,148,160,0.18)', overflow: 'hidden' }}>
                      ...
                      <td style={td}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            ...
                            {m.email && <div style={{ fontSize: 11, color: '#6E7278' }}>{m.email}</div>}
                          </div>
                          {m._isMe && <span style={directBadge}>YOU</span>}
                        </div>
                      </td>
                      <td style={{ ...td, color: '#6E7278', fontFamily: 'monospace' }}>{m.user_code || '—'}</td>
                      <td style={td}><span style={chip}>{m.designation || '—'}</span></td>
                      <td style={{ ...td, color: '#55585E' }}>{m.role || '—'}</td>
                      <td style={{ ...td, color: '#55585E' }}>{m.reporting_manager || '—'}</td>
                      {showStats && <td style={{ ...td, fontWeight: 700, color: '#2F6DB5' }}>{m.leads}</td>}
                      {showStats && <td style={{ ...td, fontWeight: 700, color: '#23874A' }}>{m.closures}</td>}
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <style jsx global>{`
        ...
        .org-tree li::before, .org-tree li::after { content: ''; position: absolute; top: 0; right: 50%; width: 50%; height: 28px; border-top: 1.5px solid #C9CDD2; }
        .org-tree li::after { right: auto; left: 50%; border-left: 1.5px solid #C9CDD2; }
        .org-tree li:only-child::before, .org-tree li:only-child::after { display: none; }
        .org-tree li:only-child { padding-top: 0; }
        .org-tree li:first-child::before, .org-tree li:last-child::after { border: 0 none; }
        .org-tree li:last-child::before { border-right: 1.5px solid #C9CDD2; border-radius: 0 8px 0 0; }
        .org-tree li:first-child::after { border-radius: 8px 0 0 0; }
        .org-tree ul ul::before { content: ''; position: absolute; top: 0; left: 50%; width: 0; height: 28px; border-left: 1.5px solid #C9CDD2; }
        .org-card { position: relative; display: inline-block; width: 196px; box-sizing: border-box; padding: 14px 16px 12px; border-radius: 12px; background: #fff; border: 1px solid #ECEEF0; box-shadow: 0 6px 20px rgba(110,114,120,0.12); transition: transform .15s ease, box-shadow .15s ease; cursor: default; }
...
function accentFor(node) {
  ...
  if (node._root)                                         return '#2F6DB5';
  if (d.includes('telecaller') || d.includes('pre-sale')) return '#245A96';
  if (d.includes('cp ') || d.includes('channel'))         return '#A3671A';
  if (d.includes('stm') || d.includes('sales'))           return '#2F6DB5';
  if (d.includes('market'))                               return '#D9434B';
  if (d.includes('account') || d.includes('finance'))     return '#23874A';
  if (isManagerRole(node))                            return '#2F6DB5';
  return '#55585E';
}
  ...
  return (
    <li>
      <div className="org-card" style={{ '--accent': c }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div className="org-avatar" style={{ background: root ? `linear-gradient(135deg,${c},#1D1D1F)` : `${c}16`, color: root ? '#fff' : c }}>
            {(node.name || '?')[0].toUpperCase()}
          </div>
          <div style={{ textAlign: 'left', minWidth: 0, flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: '#1D1D1F', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{node.name || '—'}</div>
              {node._isMe && !root && <span style={{ fontSize: 8, fontWeight: 800, color: '#fff', background: '#2F6DB5', padding: '1px 5px', borderRadius: 20 }}>YOU</span>}
            </div>
        ...
        {(node._isMe || (showStats && node.leads != null)) && (
          <div style={{ display: 'flex', gap: 8, marginTop: 8, paddingTop: 8, borderTop: '1px dashed #ECEEF0' }}>
            {node._isMe ? (
              <span style={{ fontSize: 10, fontWeight: 700, color: '#2F6DB5' }}>You · {node.role || ''}</span>
            ) : (
              <>
                <span style={pill}>{node.leads} leads</span>
                <span style={{ ...pill, color: '#23874A', background: '#E9FBEA' }}>{node.closures} closed</span>
              </>
            )}
          </div>
        )}
...
const pill = { fontSize: 10, fontWeight: 700, color: '#2F6DB5', background: '#F3F9FF', padding: '2px 7px', borderRadius: 20 };
...
const td   = { padding: '11px 14px', fontSize: 13, color: '#1D1D1F' };
const chip = { fontSize: 11, fontWeight: 700, color: '#2F6DB5', background: '#F3F9FF', padding: '3px 9px', borderRadius: 6, textTransform: 'uppercase', letterSpacing: 0.3 };
const avatar = { width: 30, height: 30, borderRadius: 9, background: '#F3F9FF', color: '#2F6DB5', fontWeight: 800, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center' };
const directBadge = { fontSize: 10, fontWeight: 700, color: '#23874A', background: '#E9FBEA', padding: '2px 7px', borderRadius: 20 };
