...
export function buildLOIPdf(jsPDF, meta, v, installments, opts = {}) {
  ...
  const isTundavPdf = isIndustrialPdf && loiVariant === 'tundav';
  const isKalrav3Pdf = isKalravPdf && loiVariant === 'kalrav3';
  ...
  // Details
  const showCp = /^(reference|channel partner|other)$/i.test(meta.source);
  ...
  if (isPratishthaPdf && pb) {
    ...
    infoGrid([
      ...
      [typeLabel, typeValue], ...(showCp ? [[cpLabel, meta.cpName || '—']] : []),
  ...
  const terms = isPratishthaPdf ? PRATISHTHA_TERMS : (isEOI && isKalravPdf) ? [
    ...
    ['Legal & Other Charges', 'Payment to be done by the buyer at the time of Sale Deed or Possession. (Whichever is Earlier)'],
